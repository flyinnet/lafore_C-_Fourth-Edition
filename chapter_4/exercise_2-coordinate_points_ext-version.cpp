 ...
 * 6. Функция stoi() - переводит данные типа string в числовой тип.
 * 7. Функция to_string - Выполняет обратный перевод значения из числового типа в строковый тип string
*/
...
int main()
{
	...
	for (int i=0; i<2; i++)
	{
		...
		{
			if (!isdigit(points[i].axis_x[j]))	//Проверяем каждый символ, из масссива points по оси x, введенный пользователем, на число
			{
